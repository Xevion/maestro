package maestro.task.schematic.litematica;
...
import maestro.api.schematic.CompositeSchematic;
 ...
 *
 * <p>Note: Litematica mod is unmaintained and unavailable for modern Minecraft versions. This
 * integration is legacy code that always throws LinkageError at runtime. Schematic support may be
 * reimplemented using alternative formats in the future.
 */
public final class LitematicaBridge {
    ...
    public static Tuple<IStaticSchematic, Vec3i> getSchematic(int i) {
        ...
        for (Map.Entry<String, SubRegionPlacement> entry :
            ...
            Vec3i pos =
                    transform(
                            subPlacement.getPos(), placement.getMirror(), placement.getRotation());
            ...
            BlockState[][][] states =
                    new BlockState[Math.abs(size.getX())][Math.abs(size.getZ())]
                            [Math.abs(size.getY())];
        ...
        LitematicaPlacementSchematic composite =
                new LitematicaPlacementSchematic(placement.getName());
    ...
    private static class LitematicaPlacementSchematic extends CompositeSchematic
