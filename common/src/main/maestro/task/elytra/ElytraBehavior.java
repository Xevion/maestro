...
import static maestro.utils.FastMathKt.fastCeil;
import static maestro.utils.FastMathKt.fastFloor;
...
public final class ElytraBehavior {
    ...
    public final class PathManager {
        ...
        private void pathfindAroundObstacles() {
            ...
            if (ElytraBehavior.this.task.state != ElytraTask.State.LANDING
            ...
            if (!canSeeAny
                    ...
                    && task.state != ElytraTask.State.GET_TO_JUMP) {
                var unused =
                        this.pathRecalcSegment(OptionalInt.of(rangeEndExcl - 1))
                                .thenRun(
                                        () -> {
    ...
    /**
     * Called by {@link maestro.task.ElytraTask#onTick(boolean, boolean)} when the process is in
     * control and the player is flying
     */
    public void tick() {
    ...
    private boolean passable(int x, int y, int z, boolean ignoreLava) {
        if (ignoreLava) {
            ...
            return state.getBlock() instanceof AirBlock || MovementValidation.isLava(state);
        } else {
