...
import maestro.pathing.goals.GoalRunAway
import maestro.pathing.goals.GoalTwoBlocks
...
class GetToBlockTask(
    ...
    fun getToBlock(block: BlockOptionalMeta) {
        onLostControl()
        gettingTo = block
    ...
    fun getToBlock(block: Block) {
        getToBlock(BlockOptionalMeta(block))
    }

    // Blacklist the closest block and its adjacent blocks
    @Synchronized
    fun blacklistClosest(): Boolean {
