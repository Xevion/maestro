...
public interface IWorldScanner {
     ...
     *
     * @param ctx The {@link IPlayerContext} containing player and world info that the scan is based
     *     upon
    ...
    default List<BlockPos> scanChunkRadius(
            ...
            int yLevelThreshold,
            int maxSearchRadius) {
        return scanChunkRadius(
                ctx,
                new BlockOptionalMetaLookup(filter.toArray(new Block[0])),
                max,
                yLevelThreshold,
                maxSearchRadius);
    }
    ...
    default List<BlockPos> scanChunk(
            IPlayerContext ctx, List<Block> blocks, ChunkPos pos, int max, int yLevelThreshold) {
        return scanChunk(ctx, new BlockOptionalMetaLookup(blocks), pos, max, yLevelThreshold);
    }

    /**
     * Overload of {@link #repack(IPlayerContext, int)} where the value of the {@code range}
     * parameter is {@code 40}.
     *
