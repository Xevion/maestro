...
class PathingControlManager(
    ...
    private val processes = mutableSetOf<IMaestroProcess>()
    private val active = mutableListOf<IMaestroProcess>()
    private var inControlLastTick: IMaestroProcess? = null
    private var inControlThisTick: IMaestroProcess? = null
    ...
    override fun registerProcess(process: IMaestroProcess) {
    ...
    override fun mostRecentInControl(): Optional<IMaestroProcess> = Optional.ofNullable(inControlThisTick)

    override fun mostRecentCommand(): Optional<PathingCommand> = Optional.ofNullable(command)
    ...
    fun executeProcesses(): PathingCommand? {
        ...
        while (iterator.hasNext()) {
            ...
            val exec =
                proc.onTick(
                    proc == inControlLastTick && maestro.getPathingBehavior().calcFailedLastTick(),
