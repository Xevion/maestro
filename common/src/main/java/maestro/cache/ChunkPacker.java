...
import static maestro.utils.BlockStateInterface.getFromChunk;
...
public final class ChunkPacker {
    ...
    public static CachedChunk pack(LevelChunk chunk) {
        ...
        try {
            ...
            for (int y0 = 0; y0 < height / 16; y0++) {
                ...
                int yReal = y0 << 4;
                // the mapping of BlockStateContainer.getIndex from xyz to index is y << 8 | z << 4
                // | x;
    ...
    private static PathingBlockType getPathingBlockType(
            BlockState state, LevelChunk chunk, int x, int y, int z) {
        Block block = state.getBlock();
        if (MovementHelper.isWater(state)) {
            // only water source blocks are plausibly usable, flowing water should be avoided
            // FLOWING_WATER is a waterfall, it doesn't really matter and caching it as AVOID just
            // makes it look wrong
            ...
            if ((x != 15 && MovementHelper.possiblyFlowing(getFromChunk(chunk, x + 1, adjY, z)))
                    || (x != 0
                            && MovementHelper.possiblyFlowing(getFromChunk(chunk, x - 1, adjY, z)))
                    || (z != 15
                            && MovementHelper.possiblyFlowing(getFromChunk(chunk, x, adjY, z + 1)))
                    || (z != 0
                            && MovementHelper.possiblyFlowing(
                                    getFromChunk(chunk, x, adjY, z - 1)))) {
                return PathingBlockType.AVOID;
            }
        ...
        // because fences check their adjacent blocks in the world for their fence connection status
        // to determine AABB shape
        // this caused an NPE when we saved chunks on unload, because they were
        // unable to check their neighbors
