...
public final class Settings {
     ...
     * time, even though Baritone itself will not do that.
     */
     ...
     *
     * <p>Decrease to make Baritone more often consider paths that would require placing blocks
     */
     ...
     *
     * <p>Enable if you have mods adding custom fluid physics.
     */
    public final Setting<Boolean> strictLiquidCheck = new Setting<>(false);

    /**
     * Allow Baritone to fall arbitrary distances and place a water bucket beneath it. Reliability:
     * questionable.
     */
    public final Setting<Boolean> allowWaterBucketFall = new Setting<>(true);

    /**
     * Allow Baritone to assume it can walk on still water just like any other block. This
     * functionality is assumed to be provided by a separate library that might have imported
     * Baritone.
     *
     * <p>Note: This will prevent some usage of the frostwalker enchantment, like pillaring up from
     * water.
     */
     ...
     *
     * <p>Warning: if you do something janky like sneak-backplace from an ender chest, if this is
     ...
     *
     * <p>Defaults to false because this fails on constantiam. Please let me know if this is ever
     * disabled. Please.
     */
     ...
     *
     * <p>Defaults to true, but only actually takes effect if allowParkour is also true
     */
     ...
     *
     * <p>Safer than allowParkour yet still slightly unsafe, can make contact with unchecked
     ...
     *
     * <p>Actually pretty safe, much safer than diagonal descend tbh
     */
     ...
     *
     * <p>Turn this off to force it to make more staircases and less shafts
     */
     ...
     *
     * <p>If a schematic asks for a block on this mapping, all blocks on the mapped list will be
     * accepted at that location as well
     *
     * <p>Syntax same as <a
     * href="https://baritone.leijurv.com/baritone/api/Settings.html#buildSubstitutes">buildSubstitutes</a>
     */
     ...
     *
     * <p>If a schematic asks for a block on this mapping, Baritone will place the first placeable
    ...
    /**
     * If this setting is true, Baritone will never break a block that is adjacent to an unsupported
     * falling block.
     *
     * <p>I.E. it will never trigger cascading sand / gravel falls
     */
     ...
     * needed in real pathing scenarios. And they can cause Baritone to get trapped indefinitely in
     * a strange scenario.
     *
     ...
     * You know what it is
     *
     * <p>But it's very unreliable and falls off when cornering like all the time so.
     *
     * <p>It also overshoots the landing pretty much always (making contact with the next block
     * over), so be careful
     */
     ...
     *
     * <p>Doesn't make it any more dangerous compared to just normal allowParkour th
     */
     ...
     * anyway.
     *
     * <p>This helps with speed exceeding 20m/s
     */
     ...
     * 3.56)
     *
     * <p>Setting it at 3.57 or above with sprinting, or to 4.64 or above without sprinting, will
     ...
     * of slightly worse.
     *
     * <p>Finding the optimal path is worth it, so it's the default.
     */
     ...
     *
     * <p>They have a noticeable performance impact, so they default off
     *
     * <p>Specifically, building up the avoidance map on the main thread before pathing starts
     ...
     * Set to 1.0 to effectively disable this feature
     *
     * <p>Set below 1.0 to go out of your way to walk near mob spawners
     */
     ...
     * Set to 1.0 to effectively disable this feature
     *
     * <p>Set below 1.0 to go out of your way to walk near mobs
     */
    ...
    /**
     * When running a goto towards a container block (chest, ender chest, furnace, etc), right click
     * and open it once you arrive.
     */
     ...
     *
     * <p>This is much safer to leave off now, and makes pathing more efficient. More explanation in
     * the issue.
     *
     ...
     *
     * <p>Decrease for faster map operations, but higher memory usage
     */
    ...
    /**
     * Is it okay to sprint through a descend followed by a diagonal? The player overshoots the
     ...
     *
     * <p>Note: this is *not* related to the allowDiagonalDescend setting, that is a completely
     * different thing.
     */
     ...
     *
     * <p>If no valid path (length above the minimum) has been found, pathing continues up until the
     * failure timeout
     */
     ...
     * discarded
     *
     * <p>The newest chunks are kept, so that if you're moving in a straight line quickly then stop,
    ...
    /**
     * Allow chat based control of Baritone. Most likely should be disabled when Baritone is
     * imported for use in something else
     */
     ...
     * GoalXZ if {@link #renderGoalXZBeacon} is enabled)
     */
    ...
    /**
     * Renders X/Z type Goals with the vanilla beacon beam effect. Combining this with {@link
    ...
    /**
     * Break and place blocks without having to force the client-sided rotations. Requires {@link
     * #freeLook}.
     */
    ...
    /**
     * Forces the client-sided yaw rotation to an average of the last {@link #smoothLookTicks} of
     ...
     *
     * <p>At the moment this will silently set the player's rotations when using freeLook so you're
     * not sprinting in directions other than forward, which is picken up by more "advanced"
     ...
     * crashes, see <a href="https://github.com/cabaletta/baritone/issues/327">this issue</a>.
     *
     * <p>Can be very useful on servers with low render distance. After enabling, you may need to
     ...
     *
     * <p>Note that flowing water is cached as AVOID, which is rendered as lava. As you get closer,
     * you may therefore see lava falls being replaced with water falls.
     *
     * <p>SOLID is rendered as stone in the overworld, netherrack in the nether, and end stone in
     * the end
     */
     ...
     * of locations retrieved from cache is additionaly limited by {@link #maxCachedWorldScanCount}.
     */
     ...
     *
     * <p>It is recommended to keep this value low, as it dramatically increases calculation times.
     */
     ...
     *
     * <p>This can result in more efficient loading, if you set this to the render distance.
     */
    ...
    /**
     * When the cache scan gives less blocks than the maximum threshold (but still above zero), scan
     * the main world too.
     *
     * <p>Only if you have a beefy CPU and automatically mine blocks that are in cache
     */
     ...
     *
     * <p>true = build from top to bottom
     */
     ...
     *
     * <p>Never turn this on without also turning on breakFromAbove.
     */
     ...
     * <ul>
     *   <li>NONE - No rotation
     *   <li>CLOCKWISE_90 - Rotate 90° clockwise
     *   <li>CLOCKWISE_180 - Rotate 180° clockwise
     *   <li>COUNTERCLOCKWISE_90 - Rotate 270° clockwise
     * </ul>
     */
     ...
     *
     * <ul>
     *   <li>FRONT_BACK - mirror the schematic along its local x axis
     *   <li>LEFT_RIGHT - mirror the schematic along its local z axis
     * </ul>
     */
     ...
     * but doesn't anymore.
     *
     * <p>Currently only runs when either MineBehavior or FollowBehavior is active.
     *
     * <p>For example, if Baritone is doing "mine iron_ore", the instant it breaks the ore (and it
     ...
     *
     * <p>Also on cosmic prisons this should be set to true since you don't actually mine the ore it
     ...
     *
     * <p>This is disabled because it results in some weird behavior. For example, it can """see"""
     ...
     *
     * <p>For example, set followOffsetDistance to 5 and followRadius to 0 to always stay precisely
     ...
     *
     * <p>I would highly suggest leaving this setting disabled (-1).
     *
     * <p>The only valid reason I can think of enable this setting is if you are extremely low on
     * disk space and you play on multiplayer, and can't take (average) 300kb saved for every
     * 512x512 area. (note that more complicated terrain is less compressible and will take more
     * space)
     *
     * <p>However, simply discarding old chunks because they are old is inadvisable. Baritone is
     * extremely good at correcting itself and its paths as it learns new information, as new chunks
     * load. There is no scenario in which having an incorrect cache can cause Baritone to get
     * stuck, take damage, or perform any action it wouldn't otherwise, everything is rechecked once
     * the real chunk is in range.
     *
     * <p>Having a robust cache greatly improves long distance pathfinding, as it's able to go
     * around large scale obstacles before they're in render distance. In fact, when the
     * chunkCaching setting is disabled and Baritone starts anew every time, or when you enter a
     * completely new and very complicated area, it backtracks far more often because it has to
     * build up that cache from scratch. But after it's gone through an area just once, the next
     * time will have zero backtracking, since the entire area is now known and cached.
     */
    public final Setting<Long> cachedChunksExpirySeconds = new Setting<>(-1L);

    /**
     * The function that is called when Baritone will log to chat. This function can be added to via
                    ...
                    (msg) -> {
                        try {
                            final GuiMessageTag tag =
                                    useMessageTag.value ? Helper.MESSAGE_TAG : null;
    ...
    /**
     * The function that is called when Baritone will send a desktop notification. This function can
     * be added to via {@link Consumer#andThen(Consumer)} or it can completely be overriden via
     * setting {@link Setting#value};
     */
    @JavaOnly
     ...
     * via {@link Consumer#andThen(Consumer)} or it can completely be overriden via setting {@link
     * Setting#value};
     */
    @JavaOnly
    public final Setting<BiConsumer<Component, Component>> toaster =
            new Setting<>(BaritoneToast::addOrUpdate);
    ...
    /**
     * Whether nether-pathfinder should generate terrain based on {@link #elytraNetherSeed}. If
    ...
    public final Setting<Long> elytraTimeBetweenCacheCullSecs =
            new Setting<>(TimeUnit.MINUTES.toSeconds(3));
    ...
    public final class Setting<T> {
        ...
        /**
         * This should always be the same as whether the setting can be parsed from or serialized to
         * a string; in other words, the only way to modify it is by writing to {@link #value}
         * programatically.
         *
