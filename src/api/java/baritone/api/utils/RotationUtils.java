...
public final class RotationUtils {
    ...
    /**
     * Determines if the specified entity is able to reach the center of any of the sides of the
     * specified block. It first checks if the block center is reachable, and if so, that rotation
     * will be returned. If not, it will return the first center of a given side that is reachable.
     * The return type will be {@link Optional#empty()} if the entity is unable to reach any of the
     * sides of the block.
     *
     * @param ctx Context for the viewing entity
     * @param pos The target block position
    ...
    public static Optional<Rotation> reachable(
        ...
        if (BaritoneAPI.getSettings().remainWithExistingLookDirection.value
                && ctx.isLookingAt(pos)) {
            /*
             * why add 0.0001?
            ...
            if (wouldSneak) {
                ...
                HitResult result =
                        RayTraceUtils.rayTraceTowards(
                                ctx.player(), hypothetical, blockReachDistance, true);
                if (result != null
                        && result.getType() == HitResult.Type.BLOCK
                        && ((BlockHitResult) result).getBlockPos().equals(pos)) {
                    return Optional.of(
                            hypothetical); // yes, if we sneaked we would still be looking at the
                    // block
                }
            } else {
        ...
        Optional<Rotation> possibleRotation =
                reachableCenter(ctx, pos, blockReachDistance, wouldSneak);
        ...
        for (Vec3 sideOffset : BLOCK_SIDE_MULTIPLIERS) {
            double xDiff =
                    shape.min(Direction.Axis.X) * sideOffset.x
                            + shape.max(Direction.Axis.X) * (1 - sideOffset.x);
            double yDiff =
                    shape.min(Direction.Axis.Y) * sideOffset.y
                            + shape.max(Direction.Axis.Y) * (1 - sideOffset.y);
            double zDiff =
                    shape.min(Direction.Axis.Z) * sideOffset.z
                            + shape.max(Direction.Axis.Z) * (1 - sideOffset.z);
            possibleRotation =
                    reachableOffset(
                            ctx,
                            pos,
                            new Vec3(pos.getX(), pos.getY(), pos.getZ()).add(xDiff, yDiff, zDiff),
                            blockReachDistance,
                            wouldSneak);
            if (possibleRotation.isPresent()) {
                return possibleRotation;
    ...
    /**
     * Determines if the specified entity is able to reach the specified block with the given
     * offsetted position. The return type will be {@link Optional#empty()} if the entity is unable
     * to reach the block with the offset applied.
     *
     * @param ctx Context for the viewing entity
     * @param pos The target block position
    ...
    public static Optional<Rotation> reachableOffset(
        ...
        Rotation actualRotation =
                BaritoneAPI.getProvider()
                        .getBaritoneForPlayer(ctx.player())
                        .getLookBehavior()
                        .getAimProcessor()
                        .peekRotation(rotation);
        HitResult result =
                RayTraceUtils.rayTraceTowards(
                        ctx.player(), actualRotation, blockReachDistance, wouldSneak);
    ...
    /**
     * Determines if the specified entity is able to reach the specified block where it is looking
     * at the direct center of it's hitbox.
     *
     * @param ctx Context for the viewing entity
     * @param pos The target block position
     * @param blockReachDistance The block reach distance of the entity
