 ...
 */
class InventoryBehavior(maestro: Agent) : Behavior(maestro), Helper {
    ...
    fun attemptToPutOnHotbar(inMainInvy: Int, disallowedHotbar: (Int) -> Boolean): Boolean {
        val destination = getTempHotbarSlot(disallowedHotbar)
        return if (destination.isPresent) {
            requestSwapWithHotBar(inMainInvy, destination.asInt)
        } else {
            true
        }
    }
    ...
    private fun requestSwapWithHotBar(inInventory: Int, inHotbar: Int): Boolean {
    ...
    private fun bestToolAgainst(against: Block, toolClass: Class<out DiggerItem>): Int {
        val inventory = ctx.player().getInventory().items
        var bestInd = -1
        var bestSpeed = -1.0
    ...
    fun selectThrowawayForLocation(select: Boolean, x: Int, y: Int, z: Int): Boolean {
        ...
        if (throwaway(select, { stack ->
                ...
                maybe == (stack.getItem() as BlockItem).getBlock().getStateForPlacement(
                    BlockPlaceContext(object : UseOnContext(
                        ctx.world(),
                        ctx.player(),
                        InteractionHand.MAIN_HAND,
                        stack,
                        BlockHitResult(
                            Vec3(ctx.player().position().x, ctx.player().position().y, ctx.player().position().z),
                            Direction.UP,
                            ctx.playerFeet(),
                            false
                        )
