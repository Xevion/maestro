 ...
 */
class LookBehavior(maestro: Agent) : Behavior(maestro), ILookBehavior {
    /** The current look target, may be `null`.  */
    private var target: Target? = null

    /**
     * The rotation known to the server. Returned by [getEffectiveRotation] for use in
     * [IPlayerContext].
     */
    ...
    override fun updateTarget(rotation: Rotation, blockInteract: Boolean) {
        this.target = Target(
            rotation,
            ...
            EventState.POST -> {
                ...
                prevRotation?.let { prev ->
                    ...
                    when (currentTarget.mode) {
                        ...
                        Target.Mode.CLIENT -> {
                            val smoothLookEnabled = if (ctx.player().isFallFlying())
    ...
    val effectiveRotation: Optional<Rotation>
        get() = if (Agent.settings().freeLook.value) {
    ...
    private class AimProcessor(ctx: IPlayerContext) : AbstractAimProcessor(ctx) {
    ...
    private abstract class AbstractAimProcessor : ITickableAimProcessor {
        ...
        override fun fork(): ITickableAimProcessor {
            return object : AbstractAimProcessor(this@AbstractAimProcessor) {
                private var prev: Rotation = this@AbstractAimProcessor.getPrevRotation()

                override fun nextRotation(rotation: Rotation): Rotation {
                    return super.nextRotation(rotation).also { prev = it }
                }
         ...
         * increments are by `1`)
         */
        private fun nudgeToLevel(pitch: Float): Float = when {
            pitch < -20 -> pitch + 1
            pitch > 10 -> pitch - 1
            else -> pitch
        }

        private fun calculateMouseMove(current: Float, target: Float): Float {
        ...
        private fun mouseToAngle(mouseDelta: Double): Float {
            ...
            val f = ctx.minecraft().options.sensitivity().get() * 0.6 + 0.2
            return (mouseDelta * f * f * f * 8.0).toFloat() * 0.15f
        }
    }

    private class Target(val rotation: Rotation, val mode: Mode) {
        enum class Mode {
            ...
            companion object {
                fun resolve(ctx: IPlayerContext, blockInteract: Boolean): Mode {
