...
import maestro.pathing.movement.*
...
class PathingBehavior(maestro: Agent) : Behavior(maestro), IPathingBehavior {
    ...
    private fun tickPath() {
        ...
        synchronized(pathPlanLock) {
            synchronized(pathCalcLock) {
                if (inProgress != null) {
                    ...
                    if ((current == null
                        ...
                        // start, then we're ok
                        && (calcFrom != ctx.playerFeet()) && (calcFrom != expectedSegmentStart) && (currentBest.isEmpty
            ...
            if (current!!.failed() || current!!.finished()) {
                ...
                if (next != null && !next!!.path.positions().contains(ctx.playerFeet()) && !next!!.path
                        .positions()
    ...
    override fun onPlayerUpdate(event: PlayerUpdateEvent) {
        if (current != null) {
            when (event.state) {
                ...
                EventState.POST -> ctx.minecraft().options.autoJump().set(lastAutoJump)
                else -> {}
            }
        }
    }
    ...
    override fun getCurrent(): PathExecutor? {
        return current
    }

    override fun getNext(): PathExecutor? {
        return next
    }
    ...
    fun secretInternalGetCalculationContext(): CalculationContext {
        return context!!
    }
    ...
    private fun findPathInNewThread(
        start: BlockPos, talkAboutIt: Boolean, context: CalculationContext
    ) {
    ...
    private fun createPathfinder(
        start: BlockPos, goal: Goal?, previous: IPath?, context: CalculationContext
