...
public enum FasterWorldScanner implements IWorldScanner {
    ...
    public List<BlockPos> scanChunkRadius(
        ...
        return scanChunksInternal(
                ctx,
                filter,
                getChunkRange(ctx.playerFeet().x >> 4, ctx.playerFeet().z >> 4, maxSearchRadius),
                max);
    }
    ...
    private List<BlockPos> scanChunksInternal(
            IPlayerContext ctx,
    ...
    private Stream<BlockPos> scanChunkInternal(
            IPlayerContext ctx, BlockOptionalMetaLookup lookup, ChunkPos pos) {
        ChunkSource chunkProvider = ctx.world().getChunkSource();
        ...
        return collectChunkSections(
                lookup, chunkProvider.getChunk(pos.x, pos.z, false), chunkX, chunkZ, playerSectionY)
                .stream();
    }

    private List<BlockPos> collectChunkSections(
            BlockOptionalMetaLookup lookup,
            LevelChunk chunk,
            long chunkX,
            long chunkZ,
    ...
    private void visitSection(
        ...
        Palette<BlockState> palette =
                ((IPalettedContainer<BlockState>) sectionContainer).getPalette();
            ...
            if (lookup.has(palette.valueFor(0))) {
                ...
                for (int x = 0; x < 16; ++x) {
                    for (int y = 0; y < 16; ++y) {
                        for (int z = 0; z < 16; ++z) {
                            blocks.add(
                                    new BlockPos((int) chunkX + x, sectionY + y, (int) chunkZ + z));
                        }
                    }
                }
            }
    ...
    private boolean[] getIncludedFilterIndices(
            BlockOptionalMetaLookup lookup, Palette<BlockState> palette) {
