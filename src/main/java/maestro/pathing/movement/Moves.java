...
public enum Moves {
    ...
    ASCEND_NORTH(0, +1, -1) {
        ...
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            return new MovementAscend(
                    context.getMaestro(), src, new BetterBlockPos(src.x, src.y + 1, src.z - 1));
        }
    ...
    ASCEND_EAST(+1, +1, 0) {
        ...
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            return new MovementAscend(
                    context.getMaestro(), src, new BetterBlockPos(src.x + 1, src.y + 1, src.z));
        }
        ...
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            ...
            if (res.y == src.y - 1) {
                return new MovementDescend(
                        context.getMaestro(), src, new BetterBlockPos(res.x, res.y, res.z));
            } else {
    ...
    DIAGONAL_NORTHEAST(+1, 0, -1, false, true) {
        ...
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            ...
            return new MovementDiagonal(
                    context.getMaestro(), src, Direction.NORTH, Direction.EAST, res.y - src.y);
        }
    ...
    DIAGONAL_NORTHWEST(-1, 0, -1, false, true) {
        ...
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            ...
            apply(context, src.x, src.y, src.z, res);
            return new MovementDiagonal(
                    context.getMaestro(), src, Direction.NORTH, Direction.WEST, res.y - src.y);
        }
    ...
    DIAGONAL_SOUTHEAST(+1, 0, +1, false, true) {
        ...
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            ...
            apply(context, src.x, src.y, src.z, res);
            return new MovementDiagonal(
                    context.getMaestro(), src, Direction.SOUTH, Direction.EAST, res.y - src.y);
        }
    ...
    DIAGONAL_SOUTHWEST(-1, 0, +1, false, true) {
        ...
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            ...
            apply(context, src.x, src.y, src.z, res);
            return new MovementDiagonal(
                    context.getMaestro(), src, Direction.SOUTH, Direction.WEST, res.y - src.y);
        }
    ...
    // Swimming movements (3D underwater navigation)
    SWIM_NORTH(0, 0, -1) {
        @Override
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            return new MovementSwimHorizontal(context.getMaestro(), src, src.north());
        }

        @Override
        public double cost(CalculationContext context, int x, int y, int z) {
            return MovementSwimHorizontal.cost(context, x, y, z, x, y, z - 1);
        }
    },
    ...
    SWIM_SOUTH(0, 0, +1) {
        @Override
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            return new MovementSwimHorizontal(context.getMaestro(), src, src.south());
        }
    ...
    SWIM_EAST(+1, 0, 0) {
        @Override
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            return new MovementSwimHorizontal(context.getMaestro(), src, src.east());
        }

        @Override
        public double cost(CalculationContext context, int x, int y, int z) {
            return MovementSwimHorizontal.cost(context, x, y, z, x + 1, y, z);
        }
    },
    ...
    SWIM_WEST(-1, 0, 0) {
        @Override
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            return new MovementSwimHorizontal(context.getMaestro(), src, src.west());
        }
    ...
    SWIM_UP(0, +1, 0) {
        @Override
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            return new MovementSwimVertical(context.getMaestro(), src, src.above());
        }

        @Override
        public double cost(CalculationContext context, int x, int y, int z) {
            return MovementSwimVertical.cost(context, x, y, z, true);
        }
    },
    ...
    SWIM_DOWN(0, -1, 0) {
        @Override
        public Movement apply0(CalculationContext context, BetterBlockPos src) {
            return new MovementSwimVertical(context.getMaestro(), src, src.below());
        }

        @Override
        public double cost(CalculationContext context, int x, int y, int z) {
            return MovementSwimVertical.cost(context, x, y, z, false);
        }
    };
