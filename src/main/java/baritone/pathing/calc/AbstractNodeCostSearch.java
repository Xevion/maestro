...
public abstract class AbstractNodeCostSearch implements IPathFinder, Helper {
     ...
     *
     * <p>who cares about a hundredth of a tick? that's half a millisecond for crying out loud!
     */
    ...
    public synchronized PathCalculationResult calculate(long primaryTimeout, long failureTimeout) {
        ...
        try {
            IPath path =
                    calculate0(primaryTimeout, failureTimeout).map(IPath::postProcess).orElse(null);
            if (cancelRequested) {
    ...
    /**
     * Attempts to search the block position hashCode long to {@link PathNode} map for the node
     * mapped to the specified pos. If no node is found, a new node is created.
     *
     * @param x The x position of the node
     * @param y The y position of the node
     * @param z The z position of the node
     * @param hashCode The hash code of the node, provided by {@link BetterBlockPos#longHash(int,
     *     int, int)}
    ...
    public Optional<IPath> pathToMostRecentNodeConsidered() {
        return Optional.ofNullable(mostRecentConsidered)
                .map(node -> new Path(realStart, startNode, node, 0, goal, context));
    }
        ...
        for (int i = 0; i < COEFFICIENTS.length; i++) {
            ...
            if (dist
                ...
                if (logInfo) {
                    if (COEFFICIENTS[i] >= 3) {
                        System.out.println(
                                "Warning: cost coefficient is greater than three! Probably means"
                                        + " that");
                        System.out.println(
                                "the path I found is pretty terrible (like sneak-bridging for"
                                        + " dozens of blocks)");
