...
public class MovementAscend extends Movement {
    ...
    public static double cost(
        ...
        if (!MovementHelper.canWalkOn(context, destX, y, destZ, toPlace)) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                if (againstX == x
                        && againstZ
                                == z) { // we might be able to backplace now, but it doesn't matter
                    // because it will have been broken by the time we'd need to
                    // use it
                    continue;
                }
        ...
        if (context.get(x, y + 3, z).getBlock() instanceof FallingBlock
                && (MovementHelper.canWalkThrough(context, x, y + 1, z)
                        || !(srcUp2.getBlock()
                                instanceof
                                FallingBlock))) { // it would fall on us and possibly suffocate us
            // HOWEVER, we assume that we're standing in the start position
            // that means that src and src.up(1) are both air
            // maybe they aren't now, but they will be by the time this starts
            // if the lower one is can't walk through and the upper one is falling, that means that
            // by standing on src
            ...
            // and in that scenario, when we arrive and break srcUp2, that lets srcUp3 fall on us
            // and suffocate us
        }
    ...
    public MovementState updateState(MovementState state) {
        ...
        // TODO incorporate some behavior from ActionClimb (specifically how it waited until it was
        // at most 1.2 blocks away before starting to jump
        ...
        if (!MovementHelper.canWalkOn(ctx, positionToPlace, jumpingOnto)) {
            ...
            if (MovementHelper.attemptToPlaceABlock(state, baritone, dest.below(), false, true)
        ...
        double flatDistToNext =
                xAxis * Math.abs((dest.getX() + 0.5D) - ctx.player().position().x)
                        + zAxis * Math.abs((dest.getZ() + 0.5D) - ctx.player().position().z);
        double sideDist =
                zAxis * Math.abs((dest.getX() + 0.5D) - ctx.player().position().x)
                        + xAxis * Math.abs((dest.getZ() + 0.5D) - ctx.player().position().z);

        double lateralMotion =
                xAxis * ctx.player().getDeltaMovement().z
                        + zAxis * ctx.player().getDeltaMovement().x;
        ...
        // This is slightly more efficient because otherwise we might start jumping before moving,
